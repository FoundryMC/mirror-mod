...
public final class MirrorItemRenderer {
    ...
    public static void render(final ItemStack stack, final ItemDisplayContext mode, final PoseStack poseStack, final MultiBufferSource bufferSource, final int packedLight, final int packedOverlay) {
        final RenderType rendertype = ItemBlockRenderTypes.getRenderType(stack, true);
        final VertexConsumer vertexconsumer = ItemRenderer.getFoilBufferDirect(bufferSource, rendertype, true, stack.hasFoil());

        final ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();
        final ModelManager modelManager = Minecraft.getInstance().getModelManager();
        final BakedModel model = modelManager.getModel(MODEL_LOCATION);
        final BakedModel override = model.getOverrides().resolve(model, stack, null, null, 0);
        ...
        final boolean leftHand = mode == ItemDisplayContext.FIRST_PERSON_LEFT_HAND;
        final MirrorRenderer.MirrorTexture mirror = MirrorRenderer.getTexture(leftHand);
        ...
        final Vector3f look = new Vector3f(0, 0, 1);
        final Quaternionfc orientation = camera.rotation();
        final Vector3d pos = new Vector3d(cameraPos.x, cameraPos.y, cameraPos.z).add(orientation.transform(new Vector3f(leftHand ? -0.2F : 0.2F, 0.0F, -1.5F)));
        ...
        final Matrix4f matrix4f = poseStack.last().pose().invert();
...
//        MirrorRenderer.renderMirror(mirror, 0, new Vector3f(), new Vector3f(), pos.x, pos.y, pos.z, up, look, 2.0F);
        ...
        if (bufferSource instanceof final MultiBufferSource.BufferSource source) {
